...
| `agent_interface_contracts.py` | Pydantic v2 module, frozen models, `extra="forbid"` | Every schema exchanged between agents: `AdditiveIntent` union (`AddExport`/`AddRoute`/`AddProviderBinding`), `InvariantScope`, `FailureSignature`, `Finding`/`GateResult`, `RunManifest`/`Phase`/`HaltReason`, `IntentOutcome`/`IntentRejection`, and the adapter contract (`RepoDeclaration`/`GovernancePolicy`, `TestTier`, `IntentOpSpec`, `TriageRule`, `SignalSpec`, `SecretSpec`) | Single source of truth for cross-agent data shapes — prevents two agents inventing two slightly different shapes for the same thing |
