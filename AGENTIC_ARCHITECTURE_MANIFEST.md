...
| `agentic_sdlc_glossary.csv` | CSV, 4 columns (Term, Definition, Category, Tags) | 59 defined terms spanning 12 categories, from `Additive Intents` to `Zero-hit Invariant` | Formal vocabulary for the whole design set — every term used in more than one document is defined exactly once here |
