...
| F9 | C2 Checker cross-cutting note | H8 (crash recovery) | pending | C2 added `RunManifest.rejection_graph_edges` and forward-referenced H8 in prose and docstrings. Whoever executes H8 must integrate this field into whatever crash-recovery snapshot/restore procedure they define so the persisted deadlock evidence is actually read on resume, or the persistence-in-manifest becomes dead metadata. Cite `agentic-sdlc-design-v0.5.md` §4.5 and `plan/contracts/orchestration.py` `RejectionEdge`/`RunManifest.rejection_graph_edges` docstrings. |
