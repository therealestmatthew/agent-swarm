...
| F6 | C1 commit 2 (Maker observation during prose rewrite) | Housekeeping / stylistic sweep | resolved | Two references to "boundary scrubbing" survive C1 and now read ambiguously under the new design (does "boundary" mean the isolation-unit boundary or the trust boundary?): `plan/core_adapter_boundary.md:37` in the §1.1 Core-owns table and `AGENTIC_ARCHITECTURE_MANIFEST.md:66` in the row for `core_adapter_boundary.md` itself. Defensible as-is but a sweep to "egress scrubbing" would align both with the new §5 header ("Credential injection and egress scrubbing"). Resolved in `d81b007`. |
