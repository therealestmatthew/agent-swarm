...
3. **It re-opens closed questions and drops open ones.** Item 5 (Baseline Snapshot Mechanics) was
   ...
   `design/plans/versions/REGRESSION.md` exists to document, recurring one document later.
...
| D4 | `test_harness_architecture.md` §1.2; **resolved in this pass** | The mandate was `browser.new_context()` / `context.close()` — Playwright's API hard-coded into a document governing every repo. **Selenium has no `new_context()`**; its honest equivalent is a fresh driver process with a fresh profile dir, at ~1–3 **seconds** rather than milliseconds, so §1.2's "cost tradeoff, accepted deliberately" was priced for the wrong tool by two orders of magnitude. **Split** (§1.2–1.5): the *rule* (construct fresh, never clean in place) is universal and generalizes past browsers — a reused process leaks module globals, a reused connection leaks session state; the *mechanism* becomes a declared `ResetStrategy` carrying what it recreates, what host resources it needs, and what it costs. `typical_cost_ms` feeds the wall-clock ceiling and the concurrency derivation, so a two-second reset is a budget fact the pipeline reasons with. | Resolved |
| D5 | `v0.5` §9.1 vs. `test_harness_architecture.md` §3.2; **resolved in this pass** | `mutation.diff_scoped` was blocking, scoped to "files the task changed," and justified by "a pure, hermetic unit suite (this project's stated target environment)" — which silently assumed the whole repo was hermetic. **Scope predicate written** (`test_harness_architecture.md` §3.4–3.8): scope is the changed **line**, not the file, so a task is never blocked by untested code it did not touch; a line is in scope iff covered by a verified-hermetic tier; `tests.diff_covered` runs first because an uncovered line's mutant survives by construction, so mutation would otherwise be an expensive coverage check; lines covered only non-hermetically return `NOT_APPLICABLE` and take a policy branch rather than a pass; a repo with no hermetic tier makes the gate an absent capability; equivalent mutants go to a human-signed registry; the mutant ceiling halts and reports rather than sampling. | Resolved |
...
| D9 | `Finding.evidence_ref` | Nothing constrains what an evidence reference may point at, how long it is retained, or who may dereference it. The scrubbing model is settled: Core operates the scrubber at the Core/adapter trust boundary (not inside the isolation unit) and holds active values transiently in `SecretScrubberConfig` — see `core_adapter_boundary.md` §5. Retention and dereference rules are still unwritten. | Medium |
...
| D21 | `GateResult`, arising from D5 | **A `GateResult` could not say "I did not run."** `passed: bool` forces a scoped gate with nothing in scope to either report True — a green check for a check that never happened — or report False and block work it never examined. Every gate that can be scoped out of a diff has this problem; `mutation.diff_scoped` merely surfaced it first. Closed by `GateApplicability` (`applied` / `not_applicable` / `degraded`) plus `GateResult.is_green`, which PR summaries render instead of `passed`, so a scoped-out or degraded gate can never display as green. Additive, defaulting to `applied`, so existing verdict-ledger rows keep meaning what they meant. | Resolved |
| D22 | `test_harness_architecture.md` §1.2, arising from D4 | **The reset mechanism was asserted to work and never checked.** §1.2 stated that full teardown produces a clean slate — a load-bearing claim about a mechanism, in a design whose own working agreement is not to assert what has not been run. Worse, the moment resets became declarable (D4) it opened a self-rewarding loophole: declare a cheap reset, get a fast suite, and the cost lands on triage quality where it reads as flakiness rather than as a declaration. Closed by §1.5 using evidence that already existed for another purpose — a clean-state diff at t=0 *is* a failed reset, so the per-strategy diff rate audits the strategy. Above `max_baseline_diff_rate`, Core demotes the tier to the strictest declared strategy and records it. **Tightening is automatic** (costs only time); **loosening is a human gate** (costs correctness). Same shape as the conflict counter driving shared-file promotion. | Resolved |
| D18 | `v0.5` §4.5, since v0.2 | **Smart Mutex Rejection has been named in every version since v0.2 without ever having a shape.** §4.5 promises a rejection carrying "the blocking context (what the other agent already claimed), so it can resolve in one shot" — but no schema anywhere defined what that context is, which meant the design's most novel mechanism was the one piece nothing could be built against. Closed by `IntentOutcome` / `IntentRejection` in `design/plans/contracts/orchestration.py`, with a closed `reason` enum covering collision, unmapped anchor, unregistered file, undeclared op, and structural overflow. | Resolved |
...
| **S0-8a** Reset strategies + isolation derivation (**D4**, **D6**, **D22**) | `ResetStrategy` declared per tier; `ResetResource` needs derive the minimum isolation unit as a floor a repo may exceed but not undercut; per-strategy clean-state diff rate audits the strategy, auto-tightening only. Settles the container question for every browser-tier repo without a judgment call. |
...
| **S0-10** `CredentialProvider` + scrubbing model (**R16**, **R9**) | A request/grant pair: the repo declares `requested_secrets`, policy holds `granted_secrets`, and an ungranted request refuses the run. Names and scopes only, never values in either artifact. Scrubber operated by Core at the Core/adapter trust boundary (not inside the isolation unit — audit C1); every `EgressPayload` becomes a `ScrubbedEgressPayload` before anything downstream sees it. Active values live in a transient `SecretScrubberConfig` in Core memory for one task's duration. Nothing unscrubbed becomes an `evidence_ref` target or reaches network egress. Credential-bearing tasks floor to `CONTAINER` isolation (`execution_isolation.md` §5.4). |
...
| **S4-5** Evidence scrubbing at Core trust boundary (**R9**) | Ships **with** S4-4, not after. Scrubber runs in Core against every `EgressPayload` leaving the isolation unit; live-tenant credentials never leave a container un-inspected. |
...
6. ~~**D5's scope predicate, concretely.**~~ **Decided:** per-line scope against verified-hermetic
   coverage, `tests.diff_covered` first, `NOT_APPLICABLE` plus a policy branch for non-hermetic-only
...
|---|---|---|
| **Core/Adapter split** (D2, D15, D16) | `design/plans/contracts/`, `agentic-sdlc-design-v0.5.md`, `CLAUDE.md`, `AGENTIC_ARCHITECTURE_MANIFEST.md` | Contracts gained the adapter-contract block and `FailureSignature.signals`; the blueprint's Modular Reference Files table gained the new companion; both indexes updated |
| **Declaration / policy split** (D14, D17) | `design/plans/contracts/`, `core_adapter_boundary.md` | `ProjectManifest` split into `RepoDeclaration` + `GovernancePolicy`; `RunManifest` gained `declaration_digest`, `policy_digest`, `policy_adjustments`; `HaltReason` gained `adapter_policy_conflict`; boundary §3 rewritten around the self-punishing/self-rewarding test and the four field classes |
...
| **Intent outcome schema** (D18, D19) | `design/plans/contracts/` | `IntentOutcome` and `IntentRejection` added, with blocking context as structured fields and a security note on why it is never prose |
...
| **Reset strategy + isolation derivation** (D4, D6, D22) | `test_harness_architecture.md`, `execution_isolation.md`, `design/plans/contracts/`, `core_adapter_boundary.md`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.md` | Harness §1.2–1.5 rewritten (universal rule vs. declared mechanism, the cost table, declared strategies, reset verification and auto-demotion); isolation §5 rewritten as a derivation with a floor rule; `ResetResource`, `ResetStrategy`, `TestTier.reset_strategy_id`, `RepoDeclaration.reset_strategies`, `GovernancePolicy.max_baseline_diff_rate`; boundary §1.2 and §4 updated; blueprint §8 prose; 2 glossary terms |
| **Mutation scope predicate** (D5, D21) | `test_harness_architecture.md`, `design/plans/contracts/`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.md` | New `test_harness_architecture.md` §3.4–3.8 with §3.2 corrected from file-scope to line-scope; `GateApplicability` + `GateResult.applicability`/`is_green`; `GovernancePolicy` gained `non_hermetic_coverage_posture` and `max_mutants_per_task`; §9.1 gained `tests.diff_covered` and re-scoped the mutation row; §10 gained the equivalence-registry guard; roster prose on `GateResult` updated; 3 glossary terms added |
| **Budget enforcement split** (D3) | `budget_and_escalation_policy.md`, `agentic-sdlc-design-v0.5.md`, `calibration_and_measurement.md`, `agentic_sdlc_glossary.md` | New `budget_and_escalation_policy.md` §4; Principle 7 rewritten; roster split into Budget Enforcer (Deterministic) and Budget Accountant (advisory); §9.1 gate re-attributed; §3 heading de-scoped; cost-per-pair attribution re-sourced to dispatch metering; glossary term added and two rewritten |
| **Tooling** (supporting the above) | `scripts/frontmatter.py`, `scripts/check_frontmatter.py`, `scripts/sync_counts.py` | `roadmap` registered as a doc type; `companion_file_count` now selects on front-matter `doc_type` rather than "every `design/plans/*.md` that isn't a design doc", which would have miscounted this file; `live_human_gate_count` added to the registry |
