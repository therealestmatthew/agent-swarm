...
|---|---|---|
| **Core/Adapter split** (D2, D15, D16) | `plan/contracts/`, `agentic-sdlc-design-v0.5.md`, `CLAUDE.md`, `AGENTIC_ARCHITECTURE_MANIFEST.md` | Contracts gained the adapter-contract block and `FailureSignature.signals`; the blueprint's Modular Reference Files table gained the new companion; both indexes updated |
| **Declaration / policy split** (D14, D17) | `plan/contracts/`, `core_adapter_boundary.md` | `ProjectManifest` split into `RepoDeclaration` + `GovernancePolicy`; `RunManifest` gained `declaration_digest`, `policy_digest`, `policy_adjustments`; `HaltReason` gained `adapter_policy_conflict`; boundary §3 rewritten around the self-punishing/self-rewarding test and the four field classes |
...
| **Intent outcome schema** (D18, D19) | `plan/contracts/` | `IntentOutcome` and `IntentRejection` added, with blocking context as structured fields and a security note on why it is never prose |
...
| **Reset strategy + isolation derivation** (D4, D6, D22) | `test_harness_architecture.md`, `execution_isolation.md`, `plan/contracts/`, `core_adapter_boundary.md`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.md` | Harness §1.2–1.5 rewritten (universal rule vs. declared mechanism, the cost table, declared strategies, reset verification and auto-demotion); isolation §5 rewritten as a derivation with a floor rule; `ResetResource`, `ResetStrategy`, `TestTier.reset_strategy_id`, `RepoDeclaration.reset_strategies`, `GovernancePolicy.max_baseline_diff_rate`; boundary §1.2 and §4 updated; blueprint §8 prose; 2 glossary terms |
| **Mutation scope predicate** (D5, D21) | `test_harness_architecture.md`, `plan/contracts/`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.md` | New `test_harness_architecture.md` §3.4–3.8 with §3.2 corrected from file-scope to line-scope; `GateApplicability` + `GateResult.applicability`/`is_green`; `GovernancePolicy` gained `non_hermetic_coverage_posture` and `max_mutants_per_task`; §9.1 gained `tests.diff_covered` and re-scoped the mutation row; §10 gained the equivalence-registry guard; roster prose on `GateResult` updated; 3 glossary terms added |
