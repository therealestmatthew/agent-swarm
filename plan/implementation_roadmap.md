...
| **Reset strategy + isolation derivation** (D4, D6, D22) | `test_harness_architecture.md`, `execution_isolation.md`, `plan/contracts/`, `core_adapter_boundary.md`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.csv` | Harness §1.2–1.5 rewritten (universal rule vs. declared mechanism, the cost table, declared strategies, reset verification and auto-demotion); isolation §5 rewritten as a derivation with a floor rule; `ResetResource`, `ResetStrategy`, `TestTier.reset_strategy_id`, `RepoDeclaration.reset_strategies`, `GovernancePolicy.max_baseline_diff_rate`; boundary §1.2 and §4 updated; blueprint §8 prose; 2 glossary terms |
| **Mutation scope predicate** (D5, D21) | `test_harness_architecture.md`, `plan/contracts/`, `agentic-sdlc-design-v0.5.md`, `agentic_sdlc_glossary.csv` | New `test_harness_architecture.md` §3.4–3.8 with §3.2 corrected from file-scope to line-scope; `GateApplicability` + `GateResult.applicability`/`is_green`; `GovernancePolicy` gained `non_hermetic_coverage_posture` and `max_mutants_per_task`; §9.1 gained `tests.diff_covered` and re-scoped the mutation row; §10 gained the equivalence-registry guard; roster prose on `GateResult` updated; 3 glossary terms added |
